-- ============================================================
-- BRQ – Frota Interna :: Migration v7.2
-- Garante que approve_user SEMPRE cria/garante o motorista,
-- tanto para tipo 'admin' quanto 'usuario'.
-- (Re-aplica a função com EXCEPTION handler explícito.)
-- ============================================================
...
begin
  ...
  -- Garante motorista para QUALQUER tipo (admin ou usuario)
  select id into _mid from public.motoristas where user_id = _user_id limit 1;
  if _mid is null then
    -- fallback se já existir motorista pelo email
    ...
    update public.motoristas
      set cargo = coalesce(cargo, _cargo_final),
          status = 'ativo'
      where id = _mid;
  end if;
